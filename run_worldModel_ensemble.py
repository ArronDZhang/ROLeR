...
from core.inputs import SparseFeatP # for handling discrete variable
...
def get_args_all():
    ...
    parser.add_argument("--remark_pre", type=str, default="")
    parser.add_argument('--dnn_dropout', default=0, type=float) # add for rebuttal
...
def setup_world_model(args, x_columns, y_columns, ab_columns, task, task_logit_dim, is_ranking, MODEL_SAVE_PATH):
    ...
    ensemble_models = EnsembleModel(args.n_models, args.message, MODEL_SAVE_PATH, 
                                    x_columns, y_columns, task, task_logit_dim,
                                    dnn_hidden_units=args.dnn, dnn_hidden_units_var=args.dnn_var,
                                    seed=args.seed, l2_reg_dnn=args.l2_reg_dnn,
                                    device=device, ab_columns=ab_columns, dnn_dropout=args.dnn_dropout,
