...
class SparseFeatP(SparseFeat):
    def __new__(cls, name, vocabulary_size, embedding_dim=4, use_hash=False, dtype="int32", embedding_name=None,
                group_name=DEFAULT_GROUP_NAME, padding_idx=None): # padding_idx is newly added
...
def input_from_feature_columns(X, feature_columns, embedding_dict, feature_index, support_dense: bool, device):
    ...
    ## 当feature_index中只有一个键值对'feat_item':(0, 1)时, sparse_embedding_list中tensor最后一维是item emb
    ## 而这些item emb对应state tracker在初始化时加在最后一行的new embedding
...
def create_embedding_matrix(feature_columns, init_std=0.0001, linear=False, sparse=False, device='cpu'):
    ...
    for tensor in embedding_dict.values(): # 这里在对weights进行初始化
