...
class A2CPolicy_withEmbedding(A2CPolicy):
    ...
    def __init__(
        ...
        self.state_repr, self.action_emb = [], []
        self.count = 0
    ...
    def forward(
        ...
        if isinstance(logits_masked, tuple):
            dist = self.dist_fn(*logits_masked) # *会unpack该tensor，把每一行都做成distribution
        else:
            try:
    ...
    def learn(  # type: ignore
        ...
        for i in range(repeat): # repeat表示同一批data需要训几次
            # print("repeat", repeat) # always 1
            self.count += 1
            for minibatch in batch.split(batch_size, merge_last=True):
                ...
                self.state_repr.append(obs_emb)
                # print("batch_size", batch_size)
                # print("obs_emb", obs_emb)
                # if i == 4:
                    # self.state_repr = torch.cat((self.state_repr, obs_emb), 0)
                    # print(type(self.state_repr))
                    # self.state_repr.append(obs_emb)
