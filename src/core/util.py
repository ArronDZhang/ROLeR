...
def get_sorted_domination_features(df_data, df_item, is_multi_hot, yname=None, threshold=None): # obtain the (sorted) dominated feature(s) 
...
def compute_action_distance(action: np.ndarray, actions_hist: np.ndarray,
    ...
    ## compute the distance between the current action and historical actions
...
def compute_exposure(t_diff: np.ndarray, dist: np.ndarray, tau): # measure the exposure
    if tau <= 0:
        res = 0
        return res
