...
def find_topK_to_compare(lst1, lst2, k, feature1, feature2):
    ...
    for i in range(len(topk_indices)):
        ...
        weights = topk_values[i,:]
        sum_weight = weights.sum()
        # agg_feature = features.mean(0)
        # agg_feature = (features*weights.reshape(k, 1)).mean(0)
        agg_feature = (features*weights.reshape(k, 1)).mean(0) / sum_weight
