...
def vis(item_dic, emb_dic, rew_mat):
    ...
    # Check and handle if max_reward is zero
    max_reward = max(sum(rew_mat[key][item] for item in items) for key, items in item_dic.items())
    if max_reward == 0:
        print("Maximum reward is zero, cannot normalize rewards.")
        return
    ...
    for key in emb_dic:
        if key in item_dic and key in rew_mat:
            ...
            for i in range(0, len(positions), 3):
                ...
                # Handle zero division or NaNs
                normalized_reward = reward / max_reward if max_reward else 0
    ...
    plt.savefig('test_set.png')  # Save before showing
    plt.show()
    plt.close()
