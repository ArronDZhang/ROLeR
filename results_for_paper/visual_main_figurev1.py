...
#     methods_list = ['ROLeR', 'DORL', 'MOPO', 'MBPO', 'IPS', 'BCQ', 'CQL', 'CRR', 'SQN', "UCB", r'$\epsilon$-greedy']
...
def visual4(df1, df2, save_fig_dir, savename="three"):
    ...
    # Define the new method and specify where it should be included
    new_method_for_kuairec = "CIRS"
    
    # Initial methods list without the new method
    methods_list_base = ['ROLeR', 'DORL', 'MOPO', 'MBPO', 'IPS', 'BCQ', 'CQL', 'CRR', 'SQN', "UCB", r'$\epsilon$-greedy']
    ...
    for index in range(len(dfs)):
        # Adjust methods list based on the dataset
        methods_list = methods_list_base.copy()
        if dataset[index] == "KuaiRec":
            # Include the new method only for "KuaiRec"
            methods_list.append(new_method_for_kuairec)
        
        df = dfs[index]

        # Ensure the data frame includes the methods you want to visualize
        # This is just a safeguard and might need adjustment based on your DataFrame structure
        df_methods = df.columns.get_level_values(1).unique()
        methods_list = [m for m in methods_list if m in df_methods]
        ...
        methods_list = sorted(methods_list, key=lambda x: methods_list_base.index(x) if x in methods_list_base else len(methods_list_base))
        methods_order = dict(zip(methods_list, range(len(methods_list))))

        df.sort_index(axis=1, key=lambda col: [methods_order[x] for x in col.to_list()], level=1, inplace=True)

        for col_index, col_name in enumerate(visual_cols):
            data = df[col_name]
            color = [color_kv[name] for name in data.columns]
            marker = [marker_kv[name] for name in data.columns]

            ax = plt.subplot2grid((3, 2), (col_index, index))
            data.plot(kind="line", linewidth=1, ax=ax, legend=None, color=color, markevery=int(len(data) / 10),
            ...
            plt.xticks(fontsize=10)
            plt.grid(linestyle='dashdot', linewidth=0.8)
            ax.set_title("({}) {}".format(series[index] + str(col_index + 1), dataset[index]), fontsize=fontsize, y=1.1, fontweight=400)

            if index == 0:
                ax.set_ylabel(col_name.replace("_", " ").title(), fontsize=9, fontweight=400)

        axs.append(ax)

    # Legend (Adjust as needed)
    fig.legend(handles=[plt.Line2D([0], [0], color=color_kv[m], marker=marker_kv[m], linestyle='', markersize=5) for m in methods_list],
               labels=methods_list, ncol=3, loc='upper center', bbox_to_anchor=(0.5, 1.05), fontsize=9.5)
...
def combile_two_tables(df1, df2, used_way, save_fig_dir, savename="main_result"): # 这个函数只是用来存结果整理后的LaTeX表格
    ...
    methods = ['ROLeR', 'DORL', 'MOPO', 'IPS', 'MBPO', 'BCQ', 'CQL', 'CRR', 'SQN', r'$\epsilon$-greedy', "UCB"][::-1]
