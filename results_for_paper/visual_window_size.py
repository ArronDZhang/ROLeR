...
# @Time    : 2023/12/03
# @Author  : Yi Zhang
# @FileName: visual_window_size.py
# remark: 这版基本是废了
...
from visual_utils import walk_paths, organize_df, loaddata_new, create_dir, handle_table
...
def visual4(df1, window_sizes, save_fig_dir, savename="three"):
    ...
    adjusted_window_sizes = {key.strip('[]'): value for key, value in window_sizes.items()}
    unique_methods = set(col[1] for col in df1.columns)  # Extract unique method names
    updated_labels = {}
    for method in unique_methods:
        window_size = adjusted_window_sizes.get(method, 'N/A')
        label = f"{method} (Window: {window_size})"
        updated_labels[method] = label
    ...
    for index in range(len(dfs)):
        ...
        ax1 = plt.subplot2grid((3, 1), (0, index))
        ...
        ax2 = plt.subplot2grid((3, 1), (1, index))
        ...
        ax3 = plt.subplot2grid((3, 1), (2, index))
    ...
    new_labels = [updated_labels.get(label, label) for label in labels]
    ax1.legend(handles=handles, labels=new_labels, ncol=5,
...
def visual_two_groups():
    ...
    dirpath = "./results_all/window_size"

    ways = {'FB', 'NX_0_', 'NX_10_'}
    metrics = {'ctr', 'len_tra', 'R_tra', 'ifeat_feat'}

    # result_dir1 = os.path.join(dirpath, "KuaiEnv")
    result_dir1 = dirpath
    filenames = walk_paths(result_dir1)
    # print('=========================')
    # print(filenames) # 拿到文件夹中所有log文件
    # print('=========================')
    dfs1, window_size1 = loaddata_new(result_dir1, filenames) # find that it's empty, so we need to re-implement loaddata
    # import ipdb
    # ipdb.set_trace()
    df1 = organize_df(dfs1, ways, metrics)
    # print("Column names in DataFrame:", df1.columns)
