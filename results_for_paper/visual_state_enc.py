...
def visual4(df1, save_fig_dir, savename="three"):
...
def visual_two_groups():
    ...
    dirpath = "./results_all/state_enc"

    ways = {'FB', 'NX_0_', 'NX_10_'}
    metrics = {'ctr', 'len_tra', 'R_tra', 'ifeat_feat'}

    # result_dir1 = os.path.join(dirpath, "KuaiEnv")
    result_dir1 = os.path.join(dirpath)
    filenames = walk_paths(result_dir1)
    dfs1 = loaddata(result_dir1, filenames) # find that it's empty, so we need to re-implement loaddata
    ...
    visual4(df1_one, save_fig_dir, savename=savename)
